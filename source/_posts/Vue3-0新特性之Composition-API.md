...
`readonly`获取一个对象(响应式/纯对象/ref对象)并返回原始代理的只读代理，只读代理是深层次的：访问的任何嵌套的`property`也是只读的。返回的代理对象不可改变。但是传入的原始对象改变时，返回的代理对象也会相应的改变。如果传入的是`ref`对象或者`reactive`对象。那么返回的代理对象也是响应式的。
